...
func main() {
	host := os.Args[1]
	//host := "emr-worker-1.cluster-181564,emr-worker-2.cluster-181564,emr-header-1.cluster-181564"
	testTableName := "test1_" + getTimestampString()
	fmt.Println("host=", host, ", testTableName=", testTableName)
	...
	if err := ac.CreateTable(crt); err != nil {
		fmt.Println("CreateTable returned an error: %v", err)
		return
	}
	fmt.Println("create table returned success")
	...
	if err != nil {
		fmt.Println("Failed to create Scan request: %s", err)
		return
	}
	fmt.Println("NewScanStr returned success")
	...
	for {
		...
		if err != nil {
			fmt.Println("parse scan result error, err=", err)
			return
		}
		rsp = append(rsp, res)
	}
	if len(rsp) != 1 {
		fmt.Println("Meta returned %d rows for prefix '%s' , want 1", len(rsp), metaKey)
		return
	}
}
