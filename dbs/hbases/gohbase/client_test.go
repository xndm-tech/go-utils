...
func TestDxx(t *testing.T) {

	address := "hb-bp1m205ju7p5l24ey-001.hbase.rds.aliyuncs.com:9099"
	fmt.Println(address)
	...
	table := "recommend_samh:alg_nrt_als"
	...
	if "test" == "test" {
		...
		//
		fmt.Print("Get:")
		if data, err := client.Get(table, []byte("0000abeccf9ebab5:comic"), "info:recs", attributes); err != nil {
			fmt.Println(err)
		} else {
		...
		//fmt.Print("GetVer:")
		//if data, err := client.GetVer(table, []byte("0000abeccf9ebab5:comic"), "info:recs", 10, attributes); err != nil {
		//	fmt.Println(err)
		//} else {
		//	printCells(data)
