...
type ThriftDial func(addr, user, passwd string, connTimeout time.Duration) (*IdleClient, error)
...
func (p *ThriftPool) get(expire time.Time) (*IdleClient, error) {
	...
	if p.idle.Len() == 0 {
		...
		p.lock.Unlock()
		client, err := p.Dial(p.config.Addr, p.config.User, p.config.Passwd, p.config.ConnTimeout)
		if err != nil {
			atomic.AddInt32(&p.count, -1)
			return nil, err
		}
...
func (p *ThriftPool) Reconnect(client *IdleClient) (newClient *IdleClient, err error) {
	...
	newClient, err = p.Dial(p.config.Addr, p.config.User, p.config.Passwd, p.config.ConnTimeout)
	if err != nil {
		atomic.AddInt32(&p.count, -1)
		return
	}
